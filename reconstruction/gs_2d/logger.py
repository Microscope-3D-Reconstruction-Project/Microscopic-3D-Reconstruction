...
def _module_state_dict(module) -> dict:
    return (
...
class TrainingLogger:
    ...
    def log_step(
        ...
        *,
        ...
        bg_alpha_loss: Optional[Tensor] = None,
    ) -> None:
        ...
        if bg_alpha_loss is not None:
            self.writer.add_scalar("train/bg_alpha_loss", bg_alpha_loss.item(), step)
    ...
    def save_checkpoint(
        self, step: int, elapsed: float, *, pose_adjust=None, app_module=None
    ) -> None:
        ...
        if pose_adjust is not None:
            ckpt["pose_adjust"] = _module_state_dict(pose_adjust)
        if app_module is not None:
            ckpt["app_module"] = _module_state_dict(app_module)
        torch.save(ckpt, f"{self.ckpt_dir}/ckpt_{step}.pt")
    ...
    def export_ply(self, step: int, sh_degree: int, *, app_module=None) -> None:
        ...
        export_splats(
            ...
            shN=shN,
            format="ply",
            save_to=f"{self.ply_dir}/splat2dgs_{step}.ply",
        )
